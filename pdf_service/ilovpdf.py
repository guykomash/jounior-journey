...
def pdf_compress(msg_value):
    ...
    server, task, remaining_files = get_server_and_taskid(headers=headers, tool="compress")
    ...
    with open('file.pdf', 'rb') as f:
        ...
        # process(headers=headers, server=server_filename, task=)
...
def get_server_and_taskid(headers,tool):
    try:
        ...
        response_json = r.json()
        print(response_json)
        server, task, remaining_files = response_json['server'], response_json['task'], response_json['remaining_files']
        return server, task, remaining_files
    except requests.exceptions.HTTPError as err:
        print(err)
        return False,False
...
def pdf_compress_test():
    ...
    res = process(headers=headers, server=server,task=task, tool="imagepdf",server_filename=server_filename)
    if not res: return
